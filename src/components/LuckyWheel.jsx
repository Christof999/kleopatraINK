...
const DEFAULT_COLORS = [
  '#d4a537', '#1a140c', '#8b6914', '#26201a',
  '#f0c860', '#0a0806', '#b08328', '#3a2f1f',
];
...
export default function LuckyWheel({
  ...
  const slices = useMemo(() => {
    ...
    return segments.map((seg, i) => {
      ...
      return {
        path: describeSlice(50, 50, 50, startDeg, endDeg),
        ...
        >
          <svg viewBox="0 0 100 100" className="lucky-wheel-svg">
            ...
            {slices.map((s, i) => {
              ...
              return (
                <g key={`l${i}`} transform={`rotate(${s.midDeg} ${lx} ${ly})`}>
                  <text
                    x={lx} y={ly}
                    textAnchor="middle" dominantBaseline="central"
                    className="lucky-wheel-text"
                    style={{ fontSize: Math.max(3, Math.min(5.2, 6.5 - segments.length * 0.18)) + 'px' }}
                  >
                    {formatSegment(s.seg)}
                  </text>
                </g>
